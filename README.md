...
- **Account freshness**: A card on the Spends page shows each account's full data **coverage range** (earliest → latest transaction date) with a colour-coded staleness dot, so you can see both how current the data is and spot gaps
...
- **Date filtering**: Defaults to the current **financial year** (Apr 1 – Mar 31, computed dynamically) so an in-progress or incomplete trailing month doesn't skew the numbers; also This Month / Last Month / Last 30 Days / This (calendar) Year / All Time + a custom range clamped to your data
...
| Subscriptions | `GET·POST /subscription-rules`, `PATCH·DELETE /subscription-rules/{id}`, `GET /subscriptions/summary`; `GET·POST /cash-expenses` & `DELETE /cash-expenses/{id}` (recurring cash spends, auto-generated) |
