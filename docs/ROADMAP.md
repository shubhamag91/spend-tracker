...
- [x] **Fixed-spends tracker** — `subscription_rules` (name + keyword + free-form type + **frequency** + optional **min_amount**), ~25 subscription services pre-seeded; a dedicated "Fixed Spends" page groups recurring commitments by type (Rent/Electricity/Internet/Phone/OTT/AI/…) with a **per-month total** (each item normalised by its billing frequency; variable lump-sums averaged over the window). Keyword is non-unique, so one merchant string can be **split into multiple bills by amount** (e.g. parents' electricity ≥₹1,000 vs phone, both "Airtel Payments Bank"). Add rent/bills/EMIs yourself. Reporting overlay, stays counted as spend. Spans banks + cards.
