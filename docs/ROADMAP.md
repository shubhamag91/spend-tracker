...
- [x] **Fixed-spends tracker** — `subscription_rules` (name + keyword + free-form type), ~25 subscription services pre-seeded; a dedicated "Fixed Spends" page groups recurring commitments by type (Rent/Electricity/Internet/OTT/AI/…) with a per-month estimate. Add rent/bills/EMIs yourself. Reporting overlay, stays counted as spend. Spans banks + cards.
